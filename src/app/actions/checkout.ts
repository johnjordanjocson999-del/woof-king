...
export async function placeOrder(
  ...
  try {
    ...
    const order = await db.$transaction(async (tx) => {
      // Lock-ish: re-read menu items and refuse if any line overshoots.
      for (const line of basket.lines) {
        if (line.qty <= 0) continue;
        const live = await tx.menuItem.findUnique({ where: { id: line.menuItemId } });
      ...
      // Member loyalty — signed-in customers only.
      let memberCustomer =
        user?.id
          ? await tx.customer.findUnique({ where: { userId: user.id } })
          : null;
      if (!memberCustomer && user?.id && data.contactPhone) {
        memberCustomer = await tx.customer.findFirst({
          where: { phone: data.contactPhone, userId: null },
        });
        if (memberCustomer) {
          memberCustomer = await tx.customer.update({
            where: { id: memberCustomer.id },
            data: { userId: user.id },
          });
        }
      }
